    ...
    var body: some View {
        HStack(spacing: 0) {
            ...
            VStack(spacing: 0) {
                ...
                CalculatorTextView(text: valuesAcidoInfo.bioteksa.pesoEspesifico).background(Color.anatomy.bgUnderlineBeige)
                CalculatorTextView(text: valuesAcidoInfo.bioteksa.densidad).background(Color.anatomy.bgUnderlineBeige)
                CalculatorTextView(text: valuesAcidoInfo.bioteksa.riqueza).background(Color.anatomy.bgUnderlineBeige)
                CalculatorTextView(text: valuesAcidoInfo.bioteksa.HMNOL)
                CalculatorTextView(text: valuesAcidoInfo.bioteksa.HMNOL100)
            }.frame(maxWidth: .infinity).padding(.bottom)
            VStack (spacing: 0){
                CalculatorTextView(text: valuesAcidoInfo.bioteksa.medNeutrailar)
                CalculatorTextView(text: valuesAcidoInfo.otros.pesoEspesifico).background(Color.anatomy.bgUnderlineBeige)
                CalculatorTextView(text: valuesAcidoInfo.otros.densidad).background(Color.anatomy.bgUnderlineBeige)
                CalculatorTextView(text: valuesAcidoInfo.otros.riqueza).background(Color.anatomy.bgUnderlineBeige)
